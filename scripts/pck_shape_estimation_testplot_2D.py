...
# Adds shape-estimation CSV outputs (per-subplot and overall) alongside existing force table.
# New CLI: --shape-table, --shape-table-path, --shape-overall-mae, --shape-overall-mae-path, --shape-N
...
VERSION = "pck_shape_estimation_testplot_2D"
...
def build_JpS(S, L, delta, n_int=400):
    ...
    col_delta_world = R @ np.array([0.0, -X, 0.0])

    return np.column_stack([col0_world, col1_world, col2_world, col_delta_world])
...
# --- angle diff helper (wrap to [-pi, pi])
def ang_diff(a, b):
...
def interp_gt_on_unit_s(sol_gt, L, s_eval_unit):
    ...
    z_gt = np.interp(s_eval_unit, s_norm, sol_gt["z"])
    th_gt = np.interp(s_eval_unit, s_norm, sol_gt["theta"])
    return x_gt, z_gt, th_gt

def shape_metrics(S, order, L, sol_gt, Ns=200):
    """
    Returns: tip_pos_mm, tip_rot_deg, shape_pos_mm, shape_rot_deg
    """
    # TIP: integrate estimate at s={0,1}
    ...
    # SHAPE: arclength-mean errors along s in [0,1]
...
def _draw_arrow(ax, x0, z0, Fx, Fz, L, arrow_scale, color='purple'):
    ...
    zt = z0 + length * Fz / nrm
    ax.annotate(
        '', xy=(xt, zt), xytext=(x0, z0),
        arrowprops=dict(arrowstyle='-|>', lw=2.0, color=color),
        zorder=8,
        annotation_clip=False  # don't drop if tip is slightly out of bounds
    )
    # Make autoscaler “see” the arrow endpoints (for cell-tight):
...
def plot_force_grids(L, E, r, Rp, tension_cases,
    ...
    "P0_shape_pos_mm","P0_shape_rot_deg","P1_shape_pos_mm","P1_shape_rot_deg","P2_shape_pos_mm","P2_shape_rot_deg",
        ...
        if shape_overall_mae:
            with open(shape_overall_mae_path, "w", newline="") as f2:
                ...
                writer.writerow({"Method":"PCK2","MAE_tip_pos_mm":mae["P2_tip_pos_mm"],"MAE_tip_rot_deg":mae["P2_tip_rot_deg"],
    ...
    if force_table and force_records:
        ...
        mae0 = np.nanmean(np.abs(err0), axis=0); mae1 = np.nanmean(np.abs(err1), axis=0); mae2 = np.nanmean(np.abs(err2), axis=0)

        print("\n=== Force Error MAE over all configurations (N) ===")
        print(f"PCK0: MAE(Fx)={mae0[0]:.4f}, MAE(Fz)={mae0[1]:.4f}, MAE(|e|)={np.nanmean(np.linalg.norm(err0,axis=1)):.4f}")
        print(f"PCK1: MAE(Fx)={mae1[0]:.4f}, MAE(Fz)={mae1[1]:.4f}, MAE(|e|)={np.nanmean(np.linalg.norm(err1,axis=1)):.4f}")
        print(f"PCK2: MAE(Fx)={mae2[0]:.4f}, MAE(Fz)={mae2[1]:.4f}, MAE(|e|)={np.nanmean(np.linalg.norm(err2,axis=1)):.4f}")
...
def main():
    ap = argparse.ArgumentParser(description=f"PCK vs GT with optional force/shape tables. [{VERSION}]")
